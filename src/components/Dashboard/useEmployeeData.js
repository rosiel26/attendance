...
export const useEmployeeData = (user, userProfile) => {
    ...
    try {
      ...
      if (leaveData) {
        leaveData.forEach((leave) => {
          const startDate = new Date(`${leave.start_date}T00:00:00.000+08:00`);
          const endDate = new Date(`${leave.end_date}T00:00:00.000+08:00`);

          const cursor = new Date(startDate);
          const nowKey = manilaDateKey(new Date());

          while (cursor <= endDate) {
            // count only days within current Manila month
            const cursorKey = manilaDateKey(cursor);
            if (cursorKey.slice(0, 7) === nowKey.slice(0, 7)) leaveCount++;
  ...
  useEffect(() => {
    ...
    const monthlyHours = monthlyAttendance.reduce(
      (total, att) => total + (att.duration_hours || 0),
      0
    );
